...
# 이전에 사용하던 STT, MicController는 그대로 있다고 가정합니다.
# 실제 파일이 필요하다면 해당 클래스들을 코드에 포함해야 합니다.
from STT import STT
...
def run_drug_store(stt_instance):
    ...
    # --- ▼▼▼ JSON 파일 저장 로직 (파일 이름 변경) ▼▼▼ ---
    try:
        file_name = "decision.json"  # 파일 이름을 decision.json으로 변경
    ...
    recommended_text = ", ".join(recommended_medicines)
    text_to_speech(f"{recommended_text}를 추천드립니다.", SOUND_FILES["recommendation"])
    
    text_to_speech("추천드린 의약품을 드릴까요? 예 또는 아니오로 답해주세요.", SOUND_FILES["ask_if_provide"])
    user_choice = get_user_response(stt_instance)

    if user_choice == "yes":
        text_to_speech(f"{recommended_text}를 드리겠습니다.", SOUND_FILES["provide_medicine"])
    else:
        text_to_speech("알겠습니다. 도움이 필요하시면 다시 말씀해주세요.", SOUND_FILES["ask_again"])

# --- QR 코드 처방전 처리 시나리오 (변경 없음) ---
